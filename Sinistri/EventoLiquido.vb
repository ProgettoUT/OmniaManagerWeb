...
Public Class EventoLiquido
    ...
    Public Event LetturaEventoCompletata()
    ...
    Public Property NumeroPolizza() As String
        ...
        End Get
        Set(value As String)
            mNumeroPolizza = value.Replace("""", "")
            mRamo = Split(mNumeroPolizza, "/")(1)
            mPolizza = Split(mNumeroPolizza, "/")(2)
    ...
    Private Shared mWebView2 As WebView2
    Public Shared Property WebView2() As WebView2
        Get
            Return mWebView2
        End Get
        Set(value As WebView2)
            mWebView2 = value
        End Set
    End Property
    ...
    Public Async Sub LeggiPaginaEvento()
        Try
            Me.Compagnia = 1
            Me.IdSinistro = Await mWebView2.ExecuteScriptAsync(String.Format("document.getElementById('{0}').innerText;", IdStringaSinistro))
            Me.Contraente = Await mWebView2.ExecuteScriptAsync(String.Format("document.getElementById('{0}').innerText;", IdContraente))
            Me.Contraente = Me.Contraente.Replace("Contr:", "").Replace("\n", "").Replace("""", "")
            ...
            Me.NumeroPolizza = Await mWebView2.ExecuteScriptAsync(String.Format("document.getElementById('{0}').innerText;", IdPolizza))
            Me.Evento = Await mWebView2.ExecuteScriptAsync(String.Format("document.getElementById('{0}').innerText;", IdNumeroEvento))
            Dim DataSinistro As String = Await mWebView2.ExecuteScriptAsync(String.Format("document.getElementById('{0}').innerText;", IdDataSinistro))
            Me.DataSinistro = Utx.NetFunc.EstraiCaratteri(DataSinistro, "0123456789/-")
            Me.Descrizione = Await mWebView2.ExecuteScriptAsync(String.Format("document.getElementById('{0}').innerText;", IdDescrizione))
            Me.Liquidatore = Await mWebView2.ExecuteScriptAsync(String.Format("document.getElementById('{0}').innerText;", IdLiquidatore))
            Me.Ufficio = Await mWebView2.ExecuteScriptAsync(String.Format("document.getElementById('{0}').innerText;", IdUfficio))

            'If EvalScript(IdGaranzia) = "RCA" Then
            '    Me.RamoSinistro = "30"
            'End If

            Dim Stato As String = Await mWebView2.ExecuteScriptAsync(String.Format("document.getElementById('{0}').innerText;", IdStatus))
            Stato = Stato.Replace("""", "")
            If Stato.StartsWith("Aperto da") Then
                Dim Giorni As String = Utx.NetFunc.EstraiCaratteri(Stato, "0123456789")
            ...
            RaiseEvent LetturaEventoCompletata()
        Catch ex As Exception
            Globale.Log.Info(ex.Message)
            mLetturaEventoEffettuata = False
        End Try
    End Sub

    'Public Shared Function EvalScript(Id As String) As Threading.Tasks.Task(Of String)
    '    Try
    '        'Dim result As String = mWebView2.ExecuteScriptAsync(String.Format("document.getElementById('{0}').innerText;", Id))
    '        'MsgBox(1)
