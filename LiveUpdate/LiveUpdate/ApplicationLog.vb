...
Public Class ApplicationLog
        ...
        Try
            If String.IsNullOrEmpty(CartellaLog) Then
                ...
                If LogCondiviso = True Then
                    mCartellaLog = String.Format("{0}\Logs", Utx.Globale.Paths.CartellaApp)
                Else
                    mCartellaLog = String.Format("{0}\Logs\{1}", Utx.Globale.Paths.CartellaApp, Environment.UserName)
                End If
            Else
    ...
    Private Sub BoxErrore(ex As Exception, Optional VisualizzaMessaggio As Boolean = True)
        ...
        If VisualizzaMessaggio Then
            Dim Messaggio As String = String.Format("{1}{0}{0}Log: {2}",
                                     Environment.NewLine,
                                     ex.Message,
                                     Replace(mLogFile, Utx.Globale.Paths.CartellaApp, ".."))

            'controllo se è richiesto un riavvio
            If Utx.Globale.SessioneCorrente.RiavvioRichiesto = True Then
                MsgBox(String.Format("Si è verificato un errore.{0}Poiché è stato scaricato un aggiornamento un RIAVVIO (chiudi e riapri il programma) potrebbe risolvere il problema.{0}{0}Errore:{0}{1}",
                                     Environment.NewLine, Messaggio),
                       MsgBoxStyle.Exclamation, String.Format("{0}: Errore", Utx.Globale.TitoloApp))
            Else
                MsgBox(Messaggio, MsgBoxStyle.Exclamation, String.Format("{0}: Errore", Utx.Globale.TitoloApp))
            End If
        End If
    End Sub
    ...
    Private Sub BoxErrore(Messaggio As String, Optional VisualizzaMessaggio As Boolean = True)
        ...
        'se richiesto visualizza il messaggio di errore
        If VisualizzaMessaggio Then
            MsgBox(String.Format("{1}{0}{0}log: {2}", Environment.NewLine, Messaggio, Replace(mLogFile, Utx.Globale.Paths.CartellaApp, "..")),
                   MsgBoxStyle.Exclamation,
                   String.Format("{0}: Errore", Utx.Globale.TitoloApp))
        End If
    End Sub

    Public Sub LogParametri(Riga As String, ByRef Parametri As OleDb.OleDbParameterCollection)
        On Error Resume Next
        File.AppendAllText(mLogFile, Riga + Environment.NewLine)
        For k As Integer = 0 To Parametri.Count - 1
            File.AppendAllText(mLogFile, String.Format("{0:00}: {1} = |{2}|{3}",
                                                       k.ToString.PadLeft(2, "0"),
                                                       Parametri(k).ToString.Trim,
                                                       Parametri(k).Value.ToString.Trim,
                                                       Environment.NewLine))
        Next
    ...
    Public Sub LogDataTable(ByRef dt As DataTable, ByRef Campi() As String)
        ...
        For k As Integer = 0 To dt.Columns.Count - 1
            File.AppendAllText(mLogFile, String.Format("{0:00}: {1} = |{2}|",
                                                       k,
    ...
    Public Sub LogDataTable(ByRef dt As DataTable, Optional Righe As Integer = 100)
        ...
        For r As Integer = 0 To Righe
            For c As Integer = 0 To dt.Columns.Count - 1
                File.AppendAllText(mLogFile, String.Format("Riga {0:000} colonna {1:000}[{2}] = |{3}|{4}",
                                                           r, c,
                                                           dt.Columns(c).ColumnName.Trim,
                                                           dt.Rows(r).Item(c),
                                                           Environment.NewLine))
            Next
    ...
    Public Sub LogParametri(ByRef Parametri As OleDb.OleDbParameterCollection)
        On Error Resume Next
        For k As Integer = 0 To Parametri.Count - 1
            File.AppendAllText(mLogFile, String.Format("{0}: {1}={2}",
                                                       k.ToString.PadLeft(2, "0"),
                                                       Parametri(k).ToString,
                                                       Parametri(k).Value))
        Next
        File.AppendAllText(mLogFile, StrDup(40, "-"))
    End Sub
...
#Region "Log uso UT"
    Public Shared Sub LogUso(Sezione As String, Optional Salva As Boolean = False)
#If DEBUG Then
        Exit Sub
#End If
        On Error Resume Next
        sbUso.AppendLine(String.Format("{0};{1};{2};{3};{4};600;{5}",
                                       1,
                                       Utx.Globale.AgenziaRichiesta.CodiceAgenzia,
                                       Utx.Globale.UtenteCorrente.UniageUser,
                                       Left(Sezione.Trim, 255),
                                       Now,
                                       Utx.Globale.SessioneCorrente.IdSessione))

        If (sbUso.Length > 2000) OrElse (Salva = True) Then
            SalvaLogUso()
        End If
    End Sub

    Public Shared Sub SalvaLogUso()
        On Error Resume Next
        File.AppendAllText(FileLogUso, sbUso.ToString)
        sbUso.Length = 0
    End Sub

    Public Shared Function FileLogUso() As String
        Return Path.Combine(Utx.Globale.Paths.CartellaSetting, String.Format("Uso.{0}.log", Utx.Globale.UtenteCorrente.UniageUser))
    End Function
#End Region
End Class
