Namespace P01262
    Public Class P01262FE

        Protected Overrides Sub AttachEvents()
            AddHandler cmbZonaTerritoriale.SelectedIndexChanged, AddressOf ValuesChanged
        ...
        Protected Overrides Sub DetachEvents()
            RemoveHandler cmbZonaTerritoriale.SelectedIndexChanged, AddressOf ValuesChanged
        ...
        Public Sub New()
            ...
            Me.QuotatorePremio.Image = Global.UniQuota.My.Resources.Resources.P01262
            Me.docBrowser.Tag = "http://www.utools.it/Unitools/Doc/Prodotti/P01262/IndexDoc.htm"
            Me.docBrowser.Url = New System.Uri(Me.docBrowser.Tag, System.UriKind.Absolute)

            'preventivo = New SpeseMediche
            'modello = New P01262ST

            Dim decode As New P01262DE
            'preventivo.Decode = decode
            Me.decode = decode

            helpChm.CodiceProdotto = "P01262"
            ...
            AttachCombo(cmbZonaTerritoriale, decode.DecodeZonaTerritoriale)

            Panels = {TableLayoutPanel0}
        End Sub

        Protected Overrides Sub ControlsToPreventivo(ByVal diretto As Boolean)
            With TryCast(preventivo, SpeseMediche)

                If diretto Then
                    .ZonaTerritoriale = cmbZonaTerritoriale.SelectedValue
                Else
                    cmbZonaTerritoriale.SelectedValue = CInt(.ZonaTerritoriale)
                    cmbZonaTerritoriale.Enabled = True
                End If
        ...
        Private Sub TabControl1_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
            If TabControl1.SelectedTab.Equals(TabAgg) Then
                ...
                TryCast(preventivo, SpeseMediche).Aggiungiassicurato(assicurato)
        ...
        Private Sub RimuoviScheda(ByVal sender As System.Object, ByVal e As System.EventArgs)
            ...
            TryCast(preventivo, SpeseMediche).Rimuoviassicurato(assicuratoFE.assicurato)
            preventivo.ValidaECalcola()
            PreventivoToControls()
        ...
        Public Overrides Sub Apri(ByVal p As Prodotto)
            If p IsNot Nothing Then
                ...
                For Each assicurato As assicurato In TryCast(preventivo, SpeseMediche).assicurati
                    AggiungiScheda(assicurato)
                Next
