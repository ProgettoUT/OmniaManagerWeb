...
#End Region
    ...
    Private Sub FormSinistri_Load(sender As Object, e As EventArgs) Handles Me.Load
        TabSinistri.ItemSize = New Size(TabSinistri.Width / 10 - 12, 25)
        RegolaCaratteriBrowser(0) 'con il valore salvato
        WvLiquido.AcceptLanguage = "it-IT"
        ...
        AddHandler WvLiquido.NewWindow, AddressOf WvLiquido_NewWindow
    End Sub
    ...
    Private Sub WvLiquido_LoadCompleted(sender As Object, e As EO.WebBrowser.LoadCompletedEventArgs) Handles WvLiquido.LoadCompleted
        Try
            If String.IsNullOrEmpty(e.Url) Then
                Exit Sub
            End If
    ...
    Private Sub WvLiquido_NewWindow(sender As Object, e As EO.WebBrowser.NewWindowEventArgs)
        Dim p As New Process
        p.StartInfo.FileName = "chrome.exe"
        p.StartInfo.Arguments = e.TargetUrl
        p.Start()
        Exit Sub
        'vecchia versione ================================== modificata il 21/07/2022
        Dim pop As New FormPopUp("OmniaManager - Liquido")
        pop.WbLiquido.WebView = e.WebView
        pop.Show()
        e.Accepted = True
    End Sub
        ...
        Try
            ...
            Do While True
                ...
                If DateDiff(DateInterval.Second, Inizio, Now) > 15 Then
                    Exit Do
                End If
                If String.IsNullOrEmpty(EventoLiquido.EvalScript("Event-EventViewScreen-ttlBar")) = False Then
                    Exit Do
                End If
            Loop
            'apro il form per la compilazione del sinistro
            Using f As New FormEditSinistro
                f.Evento.LeggiPaginaEvento()
                f.ShowDialog()
        ...
        Try
            ...
            If StampaNote Then
                Dim Sql As String = String.Format("SELECT *,Utente + Format(DataModifica,' (dd.mm.yy - hh:mm)') AS Elenco 
                    FROM SinistriMemo AS N
                    WHERE Cast(SUBSTRING(N.IdNota,3,4) AS Int) = {0} AND 
                          Cast(SUBSTRING(N.IdNota,8,4) AS Int) = {1} AND 
                          Cast(SUBSTRING(N.IdNota,13,7) AS Int) = {2} AND
                          LEN(N.IdNota) > 16
                    ORDER BY N.DataModifica", SinistroCorrente.Agenzia, SinistroCorrente.Esercizio, SinistroCorrente.Numero)
    ...
    Public Sub VariazioniCostoMeseAnno()
        Try
            ...
            Dim QueryAP As String = String.Format("SELECT A.Compagnia,A.AgenziaSinistro,A.EsercizioSinistro,A.NumeroSinistro,
                B.UltimoAggiornamentoAP,C.Riserva
                FROM SinistriDP AS A 
                INNER JOIN 
            ...
            For Each row As DataRow In dtAC.Rows
                ...
                If rowAP IsNot Nothing Then
                    row("ValoreAnnoPrecedente") = rowAP("Riserva")
                Else
...
#End If
    ...
    Private Sub NavigaLiquido(IdOggetto As String)
        Try
            WvLiquido.EvalScript(String.Format("parent.gwUtil.fireEvent(document.getElementById('{0}').id+'_act');", IdOggetto))
    ...
    Private Sub WvLiquido_LoadFailed(sender As Object, e As LoadFailedEventArgs) Handles WvLiquido.LoadFailed
        Globale.Log.Info("{0}: {1}", {e.ErrorCode, e.ErrorMessage})
        MsgBox(e.ErrorMessage, MsgBoxStyle.Exclamation, Utx.Globale.TitoloApp)
    End Sub

    Private Sub ButtonCercaOM_Click(sender As Object, e As EventArgs) Handles ButtonCercaOM.Click
        Try
            Dim Agenzia As String = WvLiquido.EvalScript("$('div#Claim-ClaimInfoBar-combinedid div.gw-infoValue').text();").ToString.Split("/")(0)
            Agenzia = Format(Val(Agenzia), "00000")
            ...
            End If
            Dim Sinistro As String = Utx.NetFunc.EstraiCaratteri(WvLiquido.EvalScript("$('div#TabBar-ClaimTab div.gw-hasDivider').attr('aria-label');").ToString.Split("-")(3), NetFunc.Funzioni.TipoCaratteri.SoloNumerici)
