...
Public Class Pdf
    ...
    Private Const AUTOR = "Unitools"
    Private Const CREATOR = "UniQuota"
    Private Const COPYRIGHT = "© 2011 Unico"
    ...
    'personalize
    Private mRowTab() As Single
    Private mColTab() As Single

    Public WithBackColor As Boolean
    ...
    Public WriteOnly Property ScaleMode() As pdfScaleMode
        Set(New_ScaleMode As pdfScaleMode)
    ...
    Public WriteOnly Property PaperSize() As pdfPaperSize
        Set(New_PaperSize As pdfPaperSize)
    ...
    Public WriteOnly Property Orientation() As pdfPageOrientation
        Set(NewOrientation As pdfPageOrientation)
    ...
    Private Function InitObj(Optional intObj As Int16 = 0) As Int16
    ...
    Optional Align As pdfTextAlign = pdfTextAlign.pdfAlignLeft, _
    ...
    '
    Public Sub SetColorStroke(rgb As Int32)
        Dim R As Byte
        Dim G As Byte
        Dim B As Byte
        ...
        End If
    End Sub
    Public Sub SetColorFill(color As Color)
        SetColorFill(RGB(color.R, color.G, color.B))
    End Sub
    '
    Public Sub SetColorFill(rgb As Int32)
        Dim R As Byte
        Dim G As Byte
        Dim B As Byte
    ...
    '
    Public Sub SetDash(dash_on As Single, Optional dash_off As Single = 0)
    ...
    '
    Public Sub SetLineCap(W As Int16)
        If (W >= 0) And (W <= 2) Then WriteObj(ToStr(W) & " J")
    End Sub

    '
    Public Sub SetLineJoin(W As Int16)
        If (W >= 0) And (W <= 2) Then WriteObj(ToStr(W) & " j")
    End Sub
    ...
    '
    Public Sub DrawRect(x1 As Single, y1 As Single, _
        x2 As Single, y2 As Single, _
        Optional Options As pdfPathOptions = pdfPathOptions.Stroked, _
        Optional Ray As Single = 0)
        Rectangle(x1, y1, x2 - x1, y2 - y1, Options, Ray)
    End Sub
    ...
    Optional ColorSpace As pdfColorSpace = pdfColorSpace.pdfRGB) As Boolean
    ...
    Private Function StrConv(buf() As Byte) As String
    ...
    '
    Public Sub LoadImgFromArray(Name As String, _
     ByRef ImgBuf() As Byte, _
     ByRef ImgColor() As Byte, _
    ImgWidth As Int32, _
    ImgHeight As Int32, _
    ImgBPP As Byte, _
    Optional ColorSpace As pdfColorSpace = pdfColorSpace.pdfRGB)
    ...
    Optional Options As pdfFontStyle = pdfFontStyle.pdfNormal)
        Dim i As Int16
        Dim sTemp As String
    ...
    Public Sub StartObject(Name As String, Optional Options As pdfObjectType = pdfObjectType.pdfNull)
        Dim i As Int16
    ...
    Private Function ToSpace(Valore As Single) As Single
        ...
        Select Case mvarScaleMode
            ...
            Case pdfScaleMode.pdf720PxInch
                ToSpace = Valore * 0.12
        End Select
    ...
    Private Function ToStr(Valore As Object, Optional Dec As Int16 = 3) As String
    ...
    Public Function LoadImgFromBMPRecource(Name As String, _
    RecourceName As String, _
    Optional ColorSpace As pdfColorSpace = pdfColorSpace.pdfRGB) As Boolean

        Try

            Dim FileName As String = System.IO.Path.GetTempPath & "Immagine.bmp"
            ' Delete the file if it exists.
            If File.Exists(FileName) Then
                File.Delete(FileName)
            End If

            CType(My.Resources.ResourceManager.GetObject(RecourceName), System.Drawing.Bitmap).Save(FileName, System.Drawing.Imaging.ImageFormat.Bmp)
            Return LoadImgFromBMPFile(Name, FileName, ColorSpace)
    ...
    Private Sub DrawCell(x1 As Single, y1 As Single, _
                        x2 As Single, y2 As Single, _
                        strTemp As String, _
                        Optional CellColor As ReportColor = ReportColor.white, _
                        Optional Align As pdfTextAlign = pdfTextAlign.pdfAlignLeft, _
                        Optional FontName As String = "Fnt1", _
                        Optional Fontsize As Single = 7)

        If (CellColor <> ReportColor.white) And WithBackColor Then
            SetColorFill(CellColor)
            Rectangle(x1, y1, x2 - x1, y2 - y1, pdfPathOptions.Filled) '+ Stroked
            SetColorFill(0)
        End If

        If Align = pdfTextAlign.pdfAlignLeft Then
            DrawText(1 + x1, 2 + y2, strTemp, FontName, Fontsize, Align)
        ElseIf Align = pdfTextAlign.pdfAlignRight Then
            DrawText(x2 - 1, 2 + y2, strTemp, FontName, Fontsize, Align)
        Else
            DrawText((x1 + x2) / 2.0, 2.0 + y2, strTemp, FontName, Fontsize, Align)
        End If
    End Sub


    Public Sub DrawCell(x As Integer, y As Integer, _
                        strTemp As String, _
                        Optional CellColor As ReportColor = ReportColor.white, _
                        Optional Align As pdfTextAlign = pdfTextAlign.pdfAlignLeft, _
                        Optional FontName As String = "Fnt1", _
                        Optional Fontsize As Single = 7)
        DrawCell(mColTab(x - 1), mRowTab(y - 1), mColTab(x), mRowTab(y), strTemp, CellColor, Align, FontName, Fontsize)

    End Sub

    Public Sub DrawCell(x1 As Integer, y1 As Integer, _
                        ...
                        Optional FontName As String = "Fnt1", _
                        Optional Fontsize As Single = 7)
        DrawCell(mColTab(x1 - 1), mRowTab(y1 - 1), mColTab(x2), mRowTab(y2), strTemp, CellColor, Align, FontName, Fontsize)

    End Sub

    Public Sub DrawBorder(x1 As Integer, y1 As Integer, _
                        x2 As Integer, y2 As Integer)
        Rectangle(mColTab(x1 - 1), mRowTab(y1 - 1), mColTab(x2) - mColTab(x1 - 1), mRowTab(y2) - mRowTab(y1 - 1), pdfPathOptions.Stroked)
    End Sub

    Public Sub SetRowTab(Value() As Single)
        Dim i As Integer
        mRowTab = Value

        For i = 1 To UBound(Value)
            If Value(i) = 0 Then
                mRowTab(i) = mRowTab(i - 1) - 12
            Else
                mRowTab(i) = mRowTab(i - 1) - Value(i)
            End If
        Next
    End Sub

    Public Sub SetColTab(Value() As Single)
        Dim i As Integer
        mColTab = Value

        For i = 1 To UBound(Value)
            If Value(i) = 0 Then
                mColTab(i) = mColTab(i - 1) + 30
            Else
                mColTab(i) = mColTab(i - 1) + Value(i)
            End If
        Next
    End Sub
