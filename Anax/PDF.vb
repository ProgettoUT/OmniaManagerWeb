...
Public Class Pdf
    ...
    Private Const CREATOR = "UniAnag"
    Private Const COPYRIGHT = "© 2014 Unico"
    ...
    Public Sub New(PaperSize As pdfPaperSize)
        mvarFileNumber = 0
        mvarEncodeASCII85 = False
        Title = "Unitools"
        mvarScaleMode = pdfScaleMode.pdf72PxInch
        Me.PaperSize = PaperSize
        mvarOrientation = pdfPageOrientation.pdfPortrait
        mvarMargin = 0
        mvarPages = 0
        intUsedFont = 0
    ...
    Optional ColorSpace As pdfColorSpace = pdfColorSpace.pdfRGB) As Boolean
        ...
        blnFlag = LeggeBMP(FileImg, ImgBuf, ImgColor, ImgWidth, ImgHeight, ImgBPP, ColorSpace)
    ...
    Private Function ToSpace(Valore As Single) As Single
        ...
        Select Case mvarScaleMode
            ...
            Case pdfScaleMode.pdfCentimeter
                Return (Valore * 72) / 2.54

            Case pdfScaleMode.pdfMillimeter
                Return (Valore * 72) / 25.4
    ...
    Private Function ToUser(Valore As Single) As Single
        ...
        Select Case mvarScaleMode
            ...
            Case pdfScaleMode.pdfCentimeter
                Return 2.54 * (Valore / 72)

            Case pdfScaleMode.pdfMillimeter
                Return 25.4 * (Valore / 72)
    ...
    Public Function LoadImgFromBMPRecources(Name As String, _
    ...
    Optional ColorSpace As pdfColorSpace = pdfColorSpace.pdfRGB) As Boolean
        ...
        Try
            ...
            Dim img As Image = CType(My.Resources.ResourceManager.GetObject(RecourceName), System.Drawing.Image)
            Dim ms = New MemoryStream()
            img.Save(ms, System.Drawing.Imaging.ImageFormat.Bmp)
            ms.Position = 0
            Return LoadImgFromBMPFile(Name, New BinaryReader(ms), ColorSpace)

            'LoadImgFromArray(Name, ImageToByte(img), Nothing, img.Size.Width, img.Size.Height, 24, ColorSpace)
            'Return True
    ...
    Public Function ChangePixelFormat(inputImage As Bitmap, newFormat As PixelFormat) As Bitmap
        Dim bmp As Bitmap = New Bitmap(inputImage.Width, inputImage.Height, newFormat)
        Dim g As Graphics = Graphics.FromImage(bmp)
        g.DrawImage(inputImage, 0, 0)
        Return bmp
    End Function
    '
    Private Sub FileGet(fb As BinaryReader, ByRef campo As Byte())
        campo = fb.ReadBytes(campo.Length)
    End Sub

    Private Sub FileGet(fb As BinaryReader, ByRef campo As Int16)
        campo = fb.ReadInt16()
    End Sub

    Private Sub FileGet(fb As BinaryReader, ByRef campo As Int32)
        campo = fb.ReadInt32()
    End Sub

    Private Function LeggeBMP(fb As BinaryReader, _
     ByRef ImgBuf() As Byte, _
     ByRef ImgColor() As Byte, _
    ...
    Optional ColorSpace As pdfColorSpace = pdfColorSpace.pdfRGB) As Boolean
        ...
        If System.Text.Encoding.ASCII.GetString(bfType) = "BM" Then
            ...
            'FileGet(fb, TempImg, bfOffBits + 1, TempImg.Length)
            FileGet(fb, TempImg)
            fb.Close()
